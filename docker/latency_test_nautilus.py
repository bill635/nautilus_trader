#!/usr/bin/env python3
"""
币安Orderbook延迟测试脚本 - Docker + NautilusTrader版本
...
class DockerBinanceLatencyConfig(StrategyConfig, frozen=True):
    """Docker环境下的币安延迟测试配置"""
    test_symbols: List[str] = ["API3", "OM", "CFX"]
    test_duration_minutes: int = 1
    orderbook_depth: int = 5
    stats_interval_seconds: int = 10


class DockerBinanceLatencyStrategy(Strategy):
    """
    Docker环境下的币安延迟测试策略
    专门针对容器环境优化的延迟监控
    """
    
    def __init__(self, config: DockerBinanceLatencyConfig) -> None:
        super().__init__(config)
        self.config = config
        
        # 延迟数据存储
        self.latencies: Dict[str, Dict[str, List[float]]] = defaultdict(lambda: defaultdict(list))
        self.recent_latencies: Dict[str, Dict[str, deque]] = defaultdict(lambda: defaultdict(lambda: deque(maxlen=200)))
    ...
    def on_start(self) -> None:
        ...
        print(f"📊 测试配置:")
        print(f"   币种: {', '.join(self.config.test_symbols)}")
        print(f"   测试时长: {self.config.test_duration_minutes}分钟")
        print(f"   订单簿深度: {self.config.orderbook_depth}档")
        print(f"   统计间隔: {self.config.stats_interval_seconds}秒")
        print()
        ...
        for symbol in self.config.test_symbols:
            try:
                ...
                print(f"✅ 已订阅 {symbol} 现货 orderbook")
                self.subscription_success_count += 1
                ...
                print(f"✅ 已订阅 {symbol} 合约 orderbook")
                self.subscription_success_count += 1
                
            except Exception as e:
                print(f"❌ 订阅 {symbol} 失败: {e}")
        
        print(f"\n📡 成功订阅 {self.subscription_success_count} 个交易对")
        print("🚀 开始数据收集...")
        print("-" * 70)
        ...
        self.clock.set_timer(
            name="stats_timer",
            interval=timedelta(seconds=self.config.stats_interval_seconds),
            callback=self._print_statistics,
        )
        
        self.clock.set_timer(
            name="end_timer",
            interval=timedelta(minutes=self.config.test_duration_minutes),
    ...
    def on_order_book_deltas(self, deltas: OrderBookDeltas) -> None:
        ...
        try:
            ...
            if symbol and symbol in self.config.test_symbols:
                # 存储延迟数据
    ...
    def on_order_book(self, order_book: OrderBook) -> None:
        ...
        try:
            # 计算延迟
            current_time_ns = time.time_ns()
            exchange_time_ns = order_book.ts_last
            latency_ms = (current_time_ns - exchange_time_ns) / 1_000_000
    ...
    def _print_statistics(self) -> None:
        """打印实时统计信息"""
        if not self.test_start_time or not self.is_running:
            return
        ...
        for symbol in self.config.test_symbols:
            ...
            for market_type in ["spot", "futures"]:
                ...
                if recent_data:
                    ...
                    print(f"  {market_emoji} {market_name}:")
                    print(f"     消息: {total_count:,} | 平均: {avg_latency:.2f}ms | 中位: {median_latency:.2f}ms")
                    print(f"     范围: {min_latency:.2f} - {max_latency:.2f}ms")
    ...
    def _generate_final_report(self) -> None:
        ...
        print(f"\n📋 最终测试报告")
        print("=" * 80)
        print(f"🕐 测试时间: {self.test_start_time.strftime('%Y-%m-%d %H:%M:%S')} UTC")
        print(f"⏱️  测试时长: {test_duration.total_seconds():.1f}秒")
        print(f"📊 总消息数: {self.total_messages:,}")
        print(f"📡 订阅成功: {self.subscription_success_count}/6")
        
        # 生成报告内容
        report_lines = []
        report_lines.append("币安Orderbook延迟测试最终报告 (Docker + NautilusTrader)")
        ...
        report_lines.append(f"测试币种: {', '.join(self.config.test_symbols)}")
        report_lines.append("")
            ...
            for market_type in ["spot", "futures"]:
                ...
                if all_symbol_latencies:
                    ...
                    if len(all_symbol_latencies) >= 20:
                        sorted_latencies = sorted(all_symbol_latencies)
                    ...
                    print(f"     📊 分位延迟: P90={p90_latency:.3f}ms P95={p95_latency:.3f}ms P99={p99_latency:.3f}ms")
                    ...
                    report_lines.append(f"    延迟分布: <10ms({fast_count/total_count*100:.1f}%) 10-50ms({medium_count/total_count*100:.1f}%) >=50ms({slow_count/total_count*100:.1f}%)")
                    report_lines.append("")
                else:
                    market_emoji = "🏪" if market_type == "spot" else "📈"
                    market_name = "现货" if market_type == "spot" else "合约"
        ...
        filename = f"/app/reports/binance_latency_nautilus_{timestamp}.txt"
...
def main():
    ...
    print("🐳 " + "="*80)
    print("🐳 NautilusTrader Docker 币安Orderbook延迟测试工具")
    print("🐳 " + "="*80)
    ...
    try:
        ...
        config = TradingNodeConfig(
            trader_id=TraderId("DOCKER-BINANCE-LATENCY-TESTER"),
        ...
        strategy = DockerBinanceLatencyStrategy(config=strategy_config)
